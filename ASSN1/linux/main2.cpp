	...
	if (in.is_open())	// 파일이 존재해서 열리는 경우
	{
		...
		if (s == "-토핑")	// 토핑 구분 문자열인 경우
		{
			...
			while(true)	// 입력 루프
			{
				...
				token = strtok(token2, ",");	// 두 번째로 tab에 의해 쪼개진 문자열은 ingredient이므로 comma로 다시 split			
				...
				if (token2 == "") continue;		// 줄의 마지막이면 루프 처음으로 돌아감
		...
		if (fullmenu == "-에지")	// 에지 구분 문자열인 경우
		{
			...
			{				
				...
				if (index + 1 != checkstring.find("	", index + 1))	// 무스 데이터가 있는 경우 입력받는다.
				{
