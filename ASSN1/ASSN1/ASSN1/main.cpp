	...
	if (in.is_open())	// 파일이 존재해서 열리는 경우
	{
		...
		if (s == "-토핑")	// 토핑 구분 문자열인 경우
		{
			while(true)	// 입력 루프
			{
				...
				/*
				index = fullmenu.find("	");
				index2 = fullmenu.find("	", index + 1);
				index3 = fullmenu.find("	", index2 + 1);
				//index4 = fullmenu.find("	", index3 + 1);
				name = fullmenu.substr(0, index);
				cout << name << endl;
				topping->set_name(name);		// 첫 문자열은 이름
				fullingredient = fullmenu.substr(index + 1, index2 - 2);
				cindex = 0;
				i = 0;
				while (true)
				{
					cindex2 = fullingredient.find(",", cindex);
					if (cindex2 == -1)
					{
						ingredient[i] = fullingredient.substr(index, strlen(fullingredient.c_str()) - 1);
						break;
					}
					else
						ingredient[i] = fullingredient.substr(cindex, cindex2);
					i++;
					cindex = cindex2 + 1;
				}
				price = atoi((fullmenu.substr(index2 + 1, index3)).c_str());
				topping->set_price(price);
				comment = fullmenu.substr(index3 + 1, strlen(fullmenu.c_str()) - 1);
				...
				*/
				...
				//ch2 = (char*)fullmenu2.c_str();	// 복사해둔 문자열 char로 변환
				//token2 = strtok(ch2, "	");		// tab으로 다시 split
				//token2 = strtok(NULL, "	");		// 이름 부분 패스
		...
		if (fullmenu == "-에지")	// 에지 구분 문자열인 경우
		{
			...
			{				
				...
				if (index + 1 == checkstring.find("	", index + 1))
				{
					edge->add_mousse(mousse);
					//token2 = strtok(NULL, "	");		// 추가금액 부분 split
				...
				/*
				ch2 = (char*)fullmenu2.c_str();	// 복사해둔 문자열 char로 변환
				token2 = strtok(ch2, "	");		// 이름 부분 split(입력은 생략)
				if (index + 1 != checkstring.find("	", index + 1))	// 무스가 있으면 무스 부분도 split (입력은 생략)
					token2 = strtok(NULL, "	"); 
				token2 = strtok(NULL, "	");		// 추가금액 부분 split
				extra_cost = atoi(token2);		// 정수 변환
				edge->set_extra_cost(extra_cost);
				...
				edgelist->add(edge);			// list에 추가
				*/
				if (token2 == "") continue;		// 문장의 끝인 경우 루프 재시작
