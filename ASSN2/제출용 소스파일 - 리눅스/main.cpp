	...
	while (true)
	{
		...
		if (menuNum == "1")			//default game
		{
			...
			while (true)
			{
				...
				{
					if (user.getAxisX() == xAxisLength - 1)	//오른쪽 벽에 붙었는데 오른쪽으로 가려 하는 경우 다시 입력받는다
						continue;
					user.moveRight();
				}
				...
				for (int t = 0; t < torpedoNum; t++)
				{
					if (torpedoArray[t]->getTorpedoStatus() && (torpedoArray[t]->getAxisY() != 0))
						torpedoArray[t]->move();
					else if (torpedoArray[t]->getTorpedoStatus() && (torpedoArray[t]->getAxisY() == 0))		//보트가 맵 끝에 도달
					{
		...
		else if (menuNum == "2")	//custom game
		{
			...
			while (true)
			{
				cout << "X Axis : ";
				cin >> xAxisLength;
				if (!cin || xAxisLength <= 0)	//입력이 정수가 아닌 경우 및 0 이하인 경우
				{
				...
				cout << "Y Axis : ";
				cin >> yAxisLength;
				if (!cin || yAxisLength == 1 || yAxisLength <= 0)	//입력이 정수가 아닌 경우 및 0 이하인 경우 혹은 보트가 배치될 수 없는 경우(높이가 1인 경우)
				{
					cout << "잘못된 입력입니다. 다시 입력해주세요." << endl;
					cin.clear();
					cin.ignore(256, '\n');
				...
				cout << "Boats : ";
				cin >> boatNum;
				if (!cin || ((xAxisLength) * (yAxisLength - 1) < boatNum) || boatNum <= 0)	//입력이 정수가 아닌 경우
				{
					cout << "잘못된 입력입니다. 다시 입력해주세요." << endl;
					cin.clear();
					cin.ignore(256, '\n');
